...
echo "Property 12"
echo "File, Mode 0, Mode 4, Mode 6, Mode 7, Mode 8, Mode 9" >> log/times_prop12.csv
for f in *.java
do
	echo "Verifying $f"
  ~/Library/Haskell/bin/descartes -p=12 -m=0 $f +RTS -P -potmp -RTS
  time0=$(cat tmp.prof | grep "total time" | grep -o "[0-9]*\.[0-9]*")
  ~/Library/Haskell/bin/descartes -p=12 -m=4 $f +RTS -P -potmp -RTS
  ...
  echo "$f, $time0, $time4, $time6, $time7, $time8, $time9" >> log/times_prop12.csv
done

rm log/times_prop13.csv
echo "Property 13"
echo "File, Mode 0, Mode 4, Mode 6, Mode 7, Mode 8, Mode 9" >> log/times_prop13.csv
for f in *.java
do
	echo "Verifying $f"
  ~/Library/Haskell/bin/descartes -p=13 -m=0 $f +RTS -P -potmp -RTS
  time0=$(cat tmp.prof | grep "total time" | grep -o "[0-9]*\.[0-9]*")
  ~/Library/Haskell/bin/descartes -p=13 -m=4 $f +RTS -P -potmp -RTS
  time4=$(cat tmp.prof | grep "total time" | grep -o "[0-9]*\.[0-9]*")
  ...
  time9=$(cat tmp.prof | grep "total time" | grep -o "[0-9]*\.[0-9]*")
  echo "$f, $time0, $time4, $time6, $time7, $time8, $time9" >> log/times_prop13.csv
done
